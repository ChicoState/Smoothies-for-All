...
const Home = () => {
  ...
  const likePost = (id) => {
    ...
    .then(result => {
        const newData = data.map(item => {
            if (item._id === result._id) {
                // Merge all existing item fields with the result to prevent data loss
                return {...item, ...result};
            }
            return item;
        });
    ...
    .catch(err => console.error("Error in likePost:", err));
  };
  ...
  const unlikePost = (id) => {
    ...
    .then(result => {
        const newData = data.map(item => {
            if (item._id === result._id) {
                // Ensure to merge all existing item fields with the result
                return {...item, ...result};
            }
            return item;
        });
    ...
    .catch(err => console.error("Error in unlikePost:", err));
  };
